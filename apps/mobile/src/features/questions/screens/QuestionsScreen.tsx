...
export default function QuestionsScreen() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await apiFetch<Question[]>(`/questions${qs}`)
      setQuestions(data)
    } catch {
  ...
  const renderItem = ({ item }: { item: Question }) => (
    <Pressable
      style={styles.card}
      onPress={() => navigation.navigate('QuestionForm', { id: item.id })}
    ...
    >
      <View style={styles.cardHeader}>
        ...
        {item.answer_text == null && <Text style={styles.noAnswer}>no answer</Text>}
      </View>
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.emptyText}>No questions yet. Tap + to add one.</Text>
          }
        />
      )}

      <Pressable style={styles.fab} onPress={() => navigation.navigate('QuestionForm', {})}>
...
const styles = StyleSheet.create({
  ...
  filterLabel: { color: '#d4d4d4', fontSize: 14, fontWeight: '600' },
  card: {
  ...
  emptyText: { color: '#a3a3a3', textAlign: 'center', marginTop: 60, fontSize: 15 },
  fab: {
    position: 'absolute',
    bottom: 24,
    right: 24,

...
export default function QuestionFormPage() {
  ...
  const { data: existingQuestion, isLoading } = useQuestion(id ?? "");
  const createQuestionMutation = useCreateQuestion();
  const updateQuestionMutation = useUpdateQuestion();
  const refineQuestionMutation = useRefineQuestion();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing) {
        await updateQuestionMutation.mutateAsync({ id: id!, ...data });
      } else {
        await createQuestionMutation.mutateAsync(data as any);
      }
      navigate("/questions");
    } catch {
  ...
  const handleRefine = async () => {
    ...
    setRefining(true);
    try {
      const result = await refineQuestionMutation.mutateAsync({

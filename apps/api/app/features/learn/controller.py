...
from app.features.learn import repository
...
async def next_question(topic: str | None = Query(None), focus: str | None = Query(None)):
    ...
    regular_row = await repository.get_next_regular_question(
        pool, topic=topic, work_only=work_only
    )
    if regular_row:
    ...
    if include_math:
        ...
        if template_ids:
            math_row = await repository.get_due_math_template(pool, template_ids)
            ...
            if math_row:
                ...
                tried_set = await repository.get_tried_math_templates(pool, template_ids)
    ...
    if not candidates:
        random_regular = await repository.get_random_regular_question(
            pool, topic=topic, work_only=work_only
        )
        if random_regular:
...
async def _get_regular_question(pool, question_id):
    ...
    row = await repository.get_question_for_review(pool, question_id)
...
async def get_review_stats(topic: str | None = Query(None), focus: str | None = Query(None)):
    ...
    pool = await get_pool()
    work_only = (focus or "").strip().lower() == "work"
    stats = await repository.get_review_stats(pool, topic=topic, work_only=work_only)
    return stats
...
async def _submit_regular_answer(
    ...
    row = await repository.get_question_for_review(pool, body.question_id)
    if not row:
        raise HTTPException(404, "Question not found")
    ...
    await repository.update_question_sr(
        pool,
    ...
    # Store the review
    review_row = await repository.insert_review(
        pool,
        question_id=body.question_id,
        user_answer=body.user_answer,
...
async def _submit_math_answer(pool, body: SubmitUnifiedAnswer) -> UnifiedReviewResponse:
    ...
    # Fetch the question
    row = await repository.get_math_question(pool, body.question_id)
    if not row:
    ...
    await repository.upsert_math_template_progress(
        pool,
        template_type=row["template_type"],
        ease_factor=sr_result.ease_factor,
        interval_days=sr_result.interval_days,
    ...
    review_row = await repository.insert_math_review(
        pool,
        math_question_id=body.question_id,
        user_answer=user_answer,
        is_correct=grade["is_correct"],
        feedback=feedback,
    )
